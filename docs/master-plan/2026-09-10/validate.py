...
check(set(prs)==initial_prs|{'PR-085','PR-086','PR-087'},'declared scope must contain initial PR-001–084 plus bounded remediations PR-085/086 and current WP5 successor PR-087; revise intentionally for later extensions')
extensions=m.get('scope_extensions',[])
check(len(extensions)==3 and extensions[0].get('id')=='PR-085' and extensions[0].get('parent_pr')=='PR-001' and extensions[0].get('kind')=='bounded_ci_remediation' and bool(extensions[0].get('trigger')) and bool(extensions[0].get('reason')),'missing exact PR-085 extension provenance')
...
check(len(prs)==87,'expected exactly 87 registered assignments')
check(extensions[2].get('id')=='PR-087' and extensions[2].get('parent_pr')=='PR-010' and extensions[2].get('kind')=='bounded_wp5_current_technical_successor' and bool(extensions[2].get('controller_proposal')) and bool(extensions[2].get('feasibility_review_sha256')),'missing exact PR087 extension provenance')
check(prs['PR-087']['dependencies']==['PR-010'],'PR087 requires actual PR010 component input')
check(prs['PR-011']['dependencies']==['PR-009','PR-010','PR-087'],'PR011 must consume the accepted current verifier group')
check('PR-087' in prs['PR-082']['dependencies'],'release must consume PR087')
check(set(prs['PR-087']['files'])==set(['verification/wp5/v1.1.0/README.md', 'verification/wp5/v1.1.0/evidence-ledger.json', 'verification/wp5/v1.1.0/package.json', 'verification/wp5/v1.1.0/receipts/activation-status.json', 'verification/wp5/v1.1.0/receipts/delivery-wave-fixtures.json', 'verification/wp5/v1.1.0/receipts/fixture-matrix.json', 'verification/wp5/v1.1.0/receipts/legacy-lane-parity.json', 'verification/wp5/v1.1.0/receipts/r2-capture-protocol.json', 'verification/wp5/v1.1.0/receipts/request-capture-reconciliation.json', 'verification/wp5/v1.1.0/receipts/zero-payload-proof.json', 'verification/wp5/v1.1.0/tests/wp5-verification.test.mjs', 'verification/wp5/v1.1.0/tools/common.mjs', 'verification/wp5/v1.1.0/tools/validate-wp5.mjs', 'verification/wp5/v1.1.0/receipts/v1.0-package-pins.json', 'verification/wp5/v1.1.0/receipts/current-subject-policy.json', 'verification/wp5/v1.1.0/tests/current-technical.test.mjs', 'package-lock.json', 'verification/research-program/pr-087/', 'docs/research-program/handoffs/PR-087.json']),'PR087 must retain exact sixteen-file package and bounded lock/evidence scope')
...
for s in subs.values():
    ...
    check(set(s['prs'])-initial_prs==({'PR-085','PR-086'} if s['id']=='1A' else {'PR-087'} if s['id']=='2A' else set()),s['id']+' has an undeclared extension')
