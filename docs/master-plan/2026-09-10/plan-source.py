...
add(3,'1A','Make implementer handoffs and review evidence enforceable',[1,2],'R16','F01 F32',
 'docs/research-program/handoffs/ scripts/research-program/check-handoff.mjs tests/research-program/handoff.test.mjs package.json .github/pull_request_template.md',
...
add(4,'1B','Introduce field-level completeness and evidence states',[2],'R01 R02 R03 R06','F02 F04 F05 F09 F10',
 ...
 'Define field observation state|Add versioned value/evidence/applicability/attempt fields using existing contracts. Separate unsupported, missing, not-applicable, conflict, not-attempted and restricted; retain historical values on failure.|Validate fixtures where unknown is not false, an identifier has no physical unit, and an expired observation retains its historical success.',
 ...
 'Publish a machine-readable completeness view|Expose the versioned coverage artifact to offline consumers with field reasons and last-attempt timestamps. Keep aggregate scores supplemental to the full vector.|Validate whole-catalog arithmetic and a record with mixed successful, restricted and failed checks.'
...
add(5,'1B','Correct freshness and inferred-unit evidence labels',[4],'R02','F06 F07 F08',
 'packages/retrieval/tools/retrieval-core-v1.2.mjs worker/public-query-service.mjs apps/web/src/lib/researcherGuidance.ts tests/research-program/freshness-and-grain.test.mjs',
...
add(6,'1B','Account for isolated records and unusable facets',[4,5],'R01 R02','F04 F17 F24',
 'packages/retrieval/tools/catalog-contract.mjs packages/coverage/ apps/web/src/components/FacetSidebar.tsx apps/web/src/data/facets.ts',
 'Source defects are visible without making missing values useful filters.',[
 'Classify the four isolated CDC records|Record the exact validation failure and publisher identity for each. Use the versioned missing-description state if appropriate instead of inventing text or dropping identity.|The accepted/searchable/isolated counts reconcile before and after migration; each moved record has a stated compatibility reason.',
 'Make facet availability informative|Treat geography/access dimensions with no discriminatory supported values as unavailable filters with a concise explanation. Preserve an explicit include-unknown option for mixed datasets.|Unknown-only and mixed known/unknown fixtures yield correct counts and never imply national coverage.',
...
add(8,'1C','Separate wire fields from labels and scientific meaning',[7],'R02 R05','F10 F11 F12',
 'packages/identity/src/schema-catalog.mjs contracts/machine-toolkit/ scripts/research/source-extractors.mjs tests/research-program/variable-identity.test.mjs',
 'Dictionary information can coexist with exact API keys without false schema claims.',[
 'Define variable identity and semantics|Add wire_name, publisher_label, definition, source_type, observed_type, unit/applicability, code values, missingness and context binding to a versioned schema.|An identifier field accepts a reasoned not-applicable unit; a measurement missing units remains incomplete.',
 'Preserve literal dictionary extraction|Adapt CDC/Census/CMS extractors so labels, concepts and definitions occupy different fields. Record exact source pointers and transformation versions.|Census concept text does not inflate the definition count; source labels and encodings survive round-trip.',
 'Represent name mappings explicitly|Add evidence-bearing documented-name to wire-name mappings with exact, reviewed alias, ambiguous or unmatched state. Do not normalize away semantic differences automatically.|The HCRIS 11 discrepant identifiers remain unresolved mappings until payload/document context supports each match.'
...
add(9,'1C','Set evidence storage and verification operating bounds',[3,4,7,8],'R03 R14 R15 R16','F03 F13 F32 F33',
 'docs/adr/ scripts/research-program/policy.json packages/connectors/ packages/ingestion/',
 'Collection, sample retention and infrastructure decisions are explicit and cost bounded.',[
 'Reconcile the architecture decisions|Write a successor ADR for the cheapest measured control-plane/publication topology using existing PostgreSQL interfaces. Identify retained metadata storage versus any proposed aggregate sample store; preserve current public runtime initially.|List exact earlier ADR clauses superseded; no paid resource or production binding is created by a documentation decision.',
 'Define request and retention budgets|Specify source allowlists, per-source concurrency, timeout, redirect, raw/expanded-byte, row, retry and retention limits. Give aggregate previews a separate rights/privacy decision and never accept arbitrary user URLs.|Policy fixtures reject a request with no eligible source, an unbounded file or an unauthorized retention class.',
 'Implement the evidence receipt contract|Record request type, expected content, final locator, actual size/hash, source and parser identities, attempt outcome and next action without secrets.|The Census 200-HTML fixture is an access/content outcome, not success; a truncated file receipt never says fully schema-valid.'
...
add(11,'2A','Validate HTTP content and source access outcomes',[9,10],'R03','F03 F13',
 ...
 'Define content-aware response classification|Add MIME, parse-shape, redirect destination and documented login/error-page checks to existing transport handling. Separate authentication-required, unavailable, content-mismatch and transient failures.|Use the captured Census Missing Key page with HTTP 200 and show that JSON success is impossible.',
 ...
 'Persist useful attempt evidence|Store sanitized headers, response hash, expected/actual content and specific next action. Preserve last good values when a check fails.|A refresh failure changes attempt/staleness state without deleting the prior successful observation or publishing an error page as data.'
...
add(56,'6A','Serve actionable access plans and retrieval recipes',[19,33,45,55],'R04 R11','F02 F19 F20',
 ...
 'Return tested recipes and precise limits|Include method, endpoint template, parameter meanings, required credentials by name only, pagination, expected schema and latest test receipt.|A returned recipe contains no secret or source payload; it does not execute merely because a client inspected it.',
...
add(70,'7C','Write beginner learning and API quick-start guides',[41,54,60,64,66],'R12 R13','F23 F24',
 ...
 'Write access and developer quick starts|Cover public downloads, API keys, applications/DUAs, Python/R/curl examples and MCP setup. Explain which actions happen at the publisher.|A new user can follow the documented prerequisites; code examples use exact wire keys and never embed a credential.',
...
add(76,'8A','Operate refresh schedules and actionable monitoring',[12,24,30,53],'R03 R14 R15','F07 F32',
 ...
 'Add actionable operational signals|Track queue age, failed checks, schema drift, coverage loss, budget exhaustion and review backlog. Notify on meaningful changes with affected sources and next action.|Repeated unchanged failures do not generate routine notification spam; critical new loss is not hidden by aggregate success.',
