...
**Acceptance requirements:** R11. **Audit findings:** F19, F21.
...
- `packages/registry/`
- `tests/research-program/machine-assets.test.mjs`
...
### C-055-1 — Read accepted asset context collections

Map canonical product/source/releases/distributions/documentation/schemas through repository ports rather than static unknown placeholders.

**Verify:** HCRIS has positive contextual collections while an unbound product stays explicitly unknown.

### C-055-2 — Page and pin each collection

Use existing bounded collection limits/cursors and generation mapping. Preserve known-empty versus not-yet-resolved collections.

**Verify:** Pagination across a generation change returns a typed restart; limits never silently clip required context.

### C-055-3 — Verify client discovery of follow-up IDs

Exercise search to get_asset and select a real release/distribution/schema for later tools. Update exact examples in agent docs.

**Verify:** The client obtains IDs from actual output; tests do not inject fake release IDs to manufacture a positive path.

## PR acceptance

- Machine asset context agrees with the public detail page.
- Three collection states complete/partial/unknown remain distinguishable.
...
```bash
node --test tests/research-program/machine-assets.test.mjs
npm run test:worker
```
