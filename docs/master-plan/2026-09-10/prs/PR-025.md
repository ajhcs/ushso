...
**Acceptance requirements:** R05, R14. **Audit findings:** F09, F10, F11.
...
- `packages/enrichment/task-builder.mjs`
- `packages/enrichment/task.schema.json`
- `tests/research-program/residual-task.test.mjs`
...
### C-025-1 — Classify residual task types

Separate prose definition, table extraction, scientific ambiguity, missing evidence and access blockade. Only the first two are routine model extraction; scientific ambiguity is an adjudication proposal.

**Verify:** A missing publisher document is not converted into a request to guess the contents.

### C-025-2 — Chunk with evidence boundaries

Build token-bounded passages preserving table headers, page positions, field context and overlap identity. Attach exact source/release and expected output scope.

**Verify:** A multi-page table does not lose its denominator/header; overlapping chunks cannot double-count a field.

### C-025-3 — Emit task manifests and abstention paths

Record required evidence coverage, token estimate and what cannot be concluded. Send oversized or low-information cases back to parsing/collection.

**Verify:** Every accepted task has source bytes and a stable ID; incomplete context allows abstention rather than forced completion.

## PR acceptance

- No task processes the entire corpus by default.
- Models are never asked to authorize access or establish endpoint availability.
...
```bash
node --test tests/research-program/residual-task.test.mjs
```
