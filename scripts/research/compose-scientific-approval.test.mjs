...
test('composition contract accepts omitted and true scientific_approval metadata while publication stays denied', async () => {
  assert.ok(scratch, 'TMPDIR or RUNNER_TEMP scratch required');
  const root = await fs.mkdtemp(path.join(scratch, 'compose-approval-'));
  try {
    ...
    const descriptor = {format: 'ushso.dictionary-review.v1', record_id: 'record-a', generation: 'g',
      ...
      variable_count: 1, evidence: [{evidence_id: 'ev', provenance_ids: ['p']}],
      provenance: [{provenance_id: 'p', locator: 'https://example.test', content_sha256: 'c'.repeat(64)}],
      limitations: [], isolated_fields: [], pages: [{sha256: pageSha, count: 1, bytes: Buffer.byteLength(rows)}]};
    ...
    await fs.writeFile(specFile, JSON.stringify({format: 'ushso.dictionary-composition-inputs.v1', generation: 'g',
      scientific_approval: true, publication_authorized: false,
      packages: [{directory: input, manifest_sha256: hash(manifestBytes)}],
      expected_fields: [{record_id: 'record-a', file: sourceFile, sha256: hash(sourceBytes), pointer: '/variables'}]}));
    const result = await runComposition({specFile, output: path.join(root, 'output')});
    assert.equal(result.records, 1);
    const output = JSON.parse(await fs.readFile(path.join(root, 'output', 'manifest.json')));
    assert.equal(output.publication_authorized, false);
    assert.equal(Object.hasOwn(output, 'scientific_approval'), false);
  } finally {
