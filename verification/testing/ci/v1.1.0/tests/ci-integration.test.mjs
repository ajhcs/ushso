...
import { assertCurrentEvidence, assertPredecessorPreserved, buildDraft } from '../tools/validate-package.mjs'
...
test('successor inventory retains structural discovery and workspace invariants', async () => {
  ...
  assert.equal(result.package_version, '1.1.0')
  assert.equal(result.package_lock.status, 'PASS')
  assert.ok(result.verification_suites.some(item => item.path === 'verification/testing/ci/v1.1.0'))
