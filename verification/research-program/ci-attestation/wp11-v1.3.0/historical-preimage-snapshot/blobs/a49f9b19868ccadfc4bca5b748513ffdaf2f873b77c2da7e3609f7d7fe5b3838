...
const previousSuccessorPath = 'verification/wp11/v1.2.0/receipts/approved.json';
const previousSuccessorSha256 = '6ff262f67687cf4166af21e7e2e2ba610158f765516e2d597b77670ac2c3a3f5';
...
async function readPreviousSuccessor() {
  const bytes = await fs.readFile(path.join(REPO, previousSuccessorPath));
  assert(digest('', bytes) === previousSuccessorSha256, 'WP11_PREVIOUS_SUCCESSOR_CHANGED');
  const receipt = JSON.parse(bytes.toString('utf8'));
  assert(receipt.package_id === '@ushso/wp11-verification-v1.2.0@1.2.0', 'WP11_PREVIOUS_SUCCESSOR_WRONG_PACKAGE');
  assert(receipt.status === 'approved_scoped_attestation', 'WP11_PREVIOUS_SUCCESSOR_NOT_RECEIPT');
  assert(receipt.release_gate_pass === false && receipt.production_eligibility === false, 'WP11_PREVIOUS_SUCCESSOR_RELEASE_OVERCLAIM');
...
return {status:'PASS',schema_version:'ushso-wp11-technical-evidence.v1.3.0',work_package:'WP11',
 ...
 files,file_set_sha256:digest('ushso:wp11-implementation-file-set:v1.3\n',files),
