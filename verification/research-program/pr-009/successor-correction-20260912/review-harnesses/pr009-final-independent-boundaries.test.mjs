...
// Root follow-up to observed typed-shape failures, with the original36cases intact.
test('R009-I8: malformed policy collections return typed validation outcomes', async t => {
  for (const [name, mutate] of [
    ['sources object', value => { value.sources = {}; }],
    ['null source entry', value => { value.sources[0] = null; }],
    ['routes object', value => { value.sources[0].routes = {}; }],
    ['forbidden classes object', value => { value.forbidden_operation_classes = {}; }],
  ]) await t.test(name, async () => {
    const candidate = clone(policy); mutate(candidate);
    const result = await validateOperatingBoundsPolicy(candidate, policyContext);
    rejected(result, name);
    assert(Array.isArray(result.issues) && result.issues.length > 0, 'Malformed policy must retain typed validation issues');
  });
});

test('R009-I9: current observation and cached capture provenance remain distinct', async t => {
  await t.test('304 reuse preserves earlier run and connector provenance', async () => {
    const input = await inputFor('not_modified');
    input.capture_reference.run_id = 'run_pr009_cached_earlier';
    input.capture_reference.connector_version = '0.9.0';
    accepted(await strictIngestion.validateIngestionRecord('capture-reference.schema.json', input.capture_reference), 'valid historical cached capture');
    const result = await composeEvidenceReceipt(input, receiptContext);
    accepted(result, '304 receipt with current connector and earlier cached capture');
    assert.equal(result.receipt.connector_identity.connector_version, descriptor.connector_version);
    assert.equal(result.receipt.underlying_records.capture_reference.connector_version, '0.9.0');
    assert.equal(result.receipt.underlying_records.capture_reference.run_id, 'run_pr009_cached_earlier');
  });
  const baseline = await capturedReceipt();
  await negativeReceipt(t, baseline, 'captured receipt cannot change JSON evidence to HTML media', value => { value.observed_media_type = 'text/html'; });
});
