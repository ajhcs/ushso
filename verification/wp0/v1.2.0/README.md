# WP0 v1.2 successor — approval pending

This implemented successor preserves the v1.0 and v1.1 receipts. Technical tests
may pass while `npm run validate` returns `BLOCKED_APPROVAL_PENDING` (exit 2).
No prior approval is inherited. No approved receipt is presently issued.
...
Commands: `npm test --prefix verification/wp0/v1.2.0`; `npm run receipt:draft
--prefix verification/wp0/v1.2.0`; `npm run validate --prefix verification/wp0/v1.2.0`.
