...
import { buildTechnicalEvidence, validateHistoricalLanes, historicalMapping, packageId } from '../tools/verify.mjs';
...
for (const id of [packageId, '@ushso/wp11-verification-v1.1.0@1.1.0', '@ushso/ci-verification-successor-v1-1@1.1.0']) {
  test(`${id}: shared approval boundary refuses missing, stale, wrong-scope and tampered evidence`, () => {
    ...
    const evidence = Buffer.from(`TEST FIXTURE ONLY: fixture-reviewer\n${approvalStatement(draft)}`);
    const approval = { schema_version: 'ushso-successor-approval.v1.0.0', status: 'approved', package_id: id,
      subject_sha256: draft.subject_sha256, reviewer: { id: 'fixture-reviewer', role: 'authorized_release_reviewer' },
