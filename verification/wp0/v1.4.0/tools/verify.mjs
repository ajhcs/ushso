...
export const packageId = '@ushso/wp0-verification-v1.4.0@1.4.0';
const execute = promisify(execFile);
const oldRoot = 'verification/wp0/v1.0.0/receipts';
const previousSuccessorPath = 'verification/wp0/v1.3.0/receipts/approved.json';
const previousSuccessorSha256 = '4c0ebf39a168c1a58ebd24ee0995fc63a9892c4592fa284dd0c25ff229aa949f';
...
export const historicalMapping = {
  'packages/retrieval/tools/query-schema.mjs': 'verification/wp0/v1.4.0/historical-algorithms/query-schema.mjs',
  'packages/retrieval/tools/webmcp.mjs': 'verification/wp0/v1.4.0/historical-algorithms/webmcp.mjs'
};
const readJson = async relative => JSON.parse(await fs.readFile(path.join(repoRoot, relative), 'utf8'));
export async function validatePreviousSuccessor() {
  const bytes = await fs.readFile(path.join(repoRoot, previousSuccessorPath));
  assert.equal(sha256(bytes), previousSuccessorSha256, 'WP0_PREVIOUS_SUCCESSOR_CHANGED');
  const receipt = JSON.parse(bytes.toString('utf8'));
  assert.equal(receipt.package_id, '@ushso/wp0-verification-v1.3.0@1.3.0', 'WP0_PREVIOUS_SUCCESSOR_WRONG_PACKAGE');
  assert.equal(receipt.status, 'approved_scoped_attestation', 'WP0_PREVIOUS_SUCCESSOR_NOT_RECEIPT');
  assert.equal(receipt.release_gate_pass, false, 'WP0_PREVIOUS_SUCCESSOR_RELEASE_OVERCLAIM');
  assert.equal(receipt.production_eligibility, false, 'WP0_PREVIOUS_SUCCESSOR_PRODUCTION_OVERCLAIM');
...
export async function validateHistoricalLanes({ mapping = historicalMapping } = {}) {
  ...
  const temporaryBase = await verificationTempRoot();
...
export async function buildTechnicalEvidence() {
  ...
  const previousSuccessor = await validatePreviousSuccessor();
  ...
  const names = ['verification/successor-support.mjs', 'package.json', 'package-lock.json', 'tests/product-boundary.test.mjs',
    ...verificationLogic, ...await listFiles(repoRoot, 'verification/wp0/v1.4.0'), ...await listFiles(repoRoot, 'packages/retrieval/tools'), ...await listFiles(repoRoot, 'worker')];
  return { status: 'PASS', predecessors, historical, active, unaffected_prerequisites: unaffected, product_boundary: boundary,
    ...
    implementation_files: await pinFiles(repoRoot, names), previous_successor_receipt: previousSuccessor,
    open_gates: ['successor_authorized_review', 'AUTH-13', 'production_authorization'], release_ready: false };
}
