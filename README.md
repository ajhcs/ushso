...
`observatory.discover_sources` WebMCP tool. The successor machine toolkit now
exposes eight read-only machine/native WebMCP inspection tools;
`observatory.plan_research` remains disabled until its explicit planner and
human gates are closed.

- Live URL: [https://ushso.org](https://ushso.org)
- Available tools: eight read-only inspection operations under the machine
  toolkit and WebMCP surface. `observatory.plan_research` remains disabled.

Copy/paste judge prompt:

> Use `observatory.search_assets` to find public sources for studying hospital
...
The browser registers the eight inspection tools on an abortable lifecycle.
Execution uses the versioned `/api/machine/v1/*` routes on the page's origin.
The tools are read-only, validate and bound inputs and outputs, propagate
cancellation, and return deterministic published metadata. They make no
authoritative-source requests, acquire no data, perform no analysis, and do not
authorize access or submit a human workflow. `observatory.plan_research` remains
disabled. The legacy `observatory.discover_sources` alias is not registered by
default; use `observatory.search_assets` for current integrations.
