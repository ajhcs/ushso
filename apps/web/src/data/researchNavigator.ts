...
export interface PriorityResearchEvidencePacket {
  ...
  matchExplanation: string
  catalog: {
    mode: 'baseline' | 'research-candidate'
...
function indexedProfile(input: IndexedProfileInput): ResearchSourceProfile {
  ...
  return {
    ...
    facts: [
      ...
      fact('Geography', input.geography, 'publisher_documented', evidenceRefs),
      fact('Period', input.period, 'publisher_documented', evidenceRefs),
      fact('Grain', input.grain, 'ushso_observed', evidenceRefs),
      fact('Cadence', input.cadence, 'unknown', evidenceRefs),
      fact('Variables / dictionary', input.variables, 'publisher_documented', evidenceRefs),
...
const indexedProfiles: ResearchSourceProfile[] = [
  ...
  indexedProfile({
    ...
    geography: 'Publisher description says single-year estimates for all U.S. counties; the retained geography field remains unresolved.',
    ...
    cadence: 'Time-series annual estimates are described; current release cadence is not independently tested.',
  ...
  indexedProfile({
    ...
    geography: 'Publisher description says national-level rates; geographic metadata for the retained record is unresolved.',
    period: 'Current-flow provisional estimates; the exact observation period is not resolved in the retained metadata.',
    grain: 'Record unit tags include state and facility; rate denominator and reporting slice require the source technical notes.',
    cadence: 'Provisional current-flow product; publication cadence is not independently tested.',
    variables: 'Maternal death counts and rates per 100,000 live births are named; definitions and revision rules require technical notes.',
  ...
  indexedProfile({
    ...
    geography: 'Publisher description names all 50 states, D.C., and three territories; retained geography metadata is unresolved.',
