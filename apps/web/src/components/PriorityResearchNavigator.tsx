    ...
    <dl className="priority-source__facts">
      {facts.map((item) => (
        <div key={item.label}>
          ...
          <details><summary>Evidence references</summary><small>{item.evidenceRefs.join(' · ')}</small></details>
        </div>
      ))}
    </dl>
  )
...
export function PriorityResearchSource({ profileId }: { profileId: string }) {
  ...
  const detailsHref = (scrollY = 0) => datasetDetailsHref(profile.catalogRecordId!, location.pathname === '/search'
  ...
  const openDetails = (event: MouseEvent<HTMLAnchorElement>) => {
    ...
    navigate(detailsHref(window.scrollY))
  }
  return (
