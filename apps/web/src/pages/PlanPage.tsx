  ...
  return (
    <div className="plan-page">
      ...
      <main id="main-content" className="plan-page__main">
        ...
        {state.status === 'blocked' && <div className="plan-surface-state plan-surface-state--blocked" role="status" data-plan-api-enabled={BLOCKED_PLAN_FEATURE_GATE.apiPlanEnabled}><CircleSlash2 aria-hidden="true" /><p>Research plan</p><h1>Plan compilation is not available yet.</h1><p>{state.message}</p><dl><div><dt>Plan API</dt><dd>Disabled</dd></div><div><dt>Contract endpoint</dt><dd>Reserved; not served by this UI foundation</dd></div><div><dt>Product boundary</dt><dd>Recommendations and instructions only; no acquisition or analysis</dd></div><div><dt>Gated compiler</dt><dd>Disabled; not an incidental side effect of adding fields</dd></div></dl></div>}
