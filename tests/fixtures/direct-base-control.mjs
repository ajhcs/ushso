// Frozen pre-direct-base retrieval engine for differential regression only.
// Keep independent of candidate algorithm edits; shared parsing/catalog dependencies remain relative imports.
import { containsNormalizedPhrase, normalizeText, parseQuestion, recordSearchText } from '../../packages/retrieval/tools/question-parser-v1.2.mjs';
import { selectJoinRoutes, validateJoinRoute } from '../../packages/retrieval/tools/join-routes.mjs';
import { compileDiscoveryIntent } from '../../packages/retrieval/tools/intent-compiler-v1.2.mjs';
import { projectSearchDocuments } from '../../packages/retrieval/tools/search-document-v1.2.mjs';
import {
  accessDimensions,
  auditEvidence,
  dateDimensions,
...
function prepareRecordScoring(record) {
  return {
    directText: JSON.parse(JSON.stringify(normalizeText([record.title, record.description].join(' ')))),
...
function subjectScore(record, preparedSubjects, searchText, recordScoring) {
  ...
  const directTokens = recordScoring.directText.split(' ').filter(Boolean);
  const components = [];
...
function compactLexicalIndex(record, document) {
  const compact = text => [...new Set(text.split(' ').filter(Boolean).map(singularToken))].join(' ');
  return [
...
function scoreRecord(record, parsed, prepared, searchText, lexicalIndex, recordScoring) {
  ...
  const essentialConceptsSupported = prepared.essentialTerms.length > 0 && prepared.essentialTerms.every(term => tokenMatch(term, recordScoring.directText.split(' ')));
...
export function createRetrievalEngine({ records, searchDocuments, joinRoutes = [], vocabulary, corpus, namedSourceRegistry = null, catalogValidation: suppliedCatalogValidation = null }) {
  ...
  const lexicalIndexes = new Map(frozenRecords.map(record => [record.record_id,
  ...
  const recordScoring = new Map(frozenRecords.map(record => [record.record_id, prepareRecordScoring(record)]));
