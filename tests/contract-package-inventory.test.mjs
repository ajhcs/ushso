...
test('CI is least-privilege, bounded, Node 22 compatible, and contains no external mutations', async () => {
  ...
  assert.match(workflow, /node-version: ['"]22\.15\.0['"]/u)
  assert.equal((workflow.match(/npm install --global npm@11\.19\.1[^\n]*\n\s*- run: npm ci/gu) ?? []).length, 2)
  assert.equal((workflow.match(/- run: npm ls ajv --all/gu) ?? []).length, 2)
