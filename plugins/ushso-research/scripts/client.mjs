// Generated from the WebMCP HTTP client. Regenerate with scripts/generate-agent-plugin.mjs.
...
function checkResponse(value         , capability        ) {
  ...
  const quota = envelope.rate_limit
...
function appendAll(parameters                 , name        , values         ) {
  if (!Array.isArray(values)) return
  for (const value of values) append(parameters, name, value)
}

function assetPath(input            , suffix = '') {
...
function routeFor(capability        , input            ) {
  ...
  switch (capability) {
    case 'get_asset': {
      ...
      const limits = input.collection_limits
      const cursors = input.collection_cursors
...
export function createBrowserMachineToolkitClient(fetchImpl            = globalThis.fetch.bind(globalThis))                            {
