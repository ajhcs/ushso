import { containsPhrase, normalizeText, parseQuestion, recordSearchText } from './question-parser.mjs';
...
function lexicalScore(record, parsed, searchText) {
  ...
  const conceptTokens = new Set([
    ...parsed.interpretation.geographies,
    ...parsed.interpretation.subjects,
    ...parsed.interpretation.units_of_analysis
  ].flatMap(match => [match.label, ...(match.matched_aliases ?? [])])
    .flatMap(value => normalizeText(value).split(' '))
    .filter(Boolean));
  const tokens = [...new Set(parsed.normalized_question.split(' ').filter(token => token.length > 2 && !STOPWORDS.has(token) && !conceptTokens.has(token)))];
  const matched = tokens.filter(token => containsPhrase(document, token));
  return {
    matched,
    components: matched.length ? [{ kind: 'lexical', value: Math.min(15, matched.length * 3), reason: `Record text matches: ${matched.join(', ')}.`, evidence_state: 'inferred' }] : []
  };
}
