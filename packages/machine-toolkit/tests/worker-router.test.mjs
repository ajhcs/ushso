...
test('public candidate routes remain unavailable by default', async () => {
  const { row, calls, router } = setup('search.search.success');
  const response = await router.handle(new Request('https://ushso.test/api/discover', {
...
test('GET route parsing supplies exact bounded nested defaults', async () => {
  ...
  const response = await router.handle(new Request(`https://ushso.test/api/datasets/${encodeURIComponent(row.input.record_id)}?generation=${row.input.expected_generation}`));
  assert.equal(response.status, 200);
...
test('router denies cross-origin requests before service invocation', async () => {
  const { row, calls, router } = setup('search.search.success', ['search_assets']);
  const response = await router.handle(new Request('https://attacker.test/api/discover', {
    method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(row.input)
  }));
...
test('GET routes reject undeclared query parameters rather than ignoring them', async () => {
  ...
  const response = await router.handle(new Request('https://ushso.test/api/datasets/asset.a?generation=gen-2026-08-30&admin=true'));
...
test('planner route remains unavailable even in protected candidate mode', async () => {
  ...
  const response = await router.handle(new Request('https://ushso.test/api/plan', {
    method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(row.input)
  }));
  assert.equal(response.status, 404);
  assert.equal(calls.length, 0);
